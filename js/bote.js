...
class BoteManager {
    ...
    async loadData() {
        ...
        // Load bote specific collections
        const [boteCollection, ingresosCollection, cashCollection, allRepartos, allCierres] = await Promise.all([
            window.DataService.getAll('bote'),
            window.DataService.getAll('ingresos'),
            window.DataService.getAll('reembolsos_efectivo'),
            window.DataService.getAll('repartos'),
